...
export default class Player {
  ...
  onTick(elapsedTime, deltaTime) {
    ...
    const intersect = this.rayVertical.intersectObjects(
    ...
    if (horizontalRayCasterDirection) {
      ...
      if (wallIntersects.length && wallIntersects[0].distance < 0.5) {
        canMove = false;
      }
    }
  ...
  initPlayer() {
    ...
    this.direction.material.visible = false;
  ...
  initBody() {
    gltfLoader.load(
      ...
      function (gltf) {
        ...
        this.actions.iddle = this.mixer.clipAction(gltf.animations[0]);
        this.actions.jump = this.mixer.clipAction(gltf.animations[1]);
        this.actions.run = this.mixer.clipAction(gltf.animations[2]);
        this.actions.walk = this.mixer.clipAction(gltf.animations[4]);
        this.actions.twerk = this.mixer.clipAction(gltf.animations[3]);
